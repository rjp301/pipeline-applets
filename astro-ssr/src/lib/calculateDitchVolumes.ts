...
const processPoint = (
  ...
  const elevation = ditchline.elevationAtNode(geometry);

  if (elevation === undefined) return { ...surveyPoint, geometry, chainage };

  const depth = surveyPoint.z - elevation;
...
const calcVolumeChunks = (surveyPoints: PointResult[]): VolumeChunk[] => {
  const filtered = surveyPoints.filter(
    (pt) => pt.chainage !== undefined && pt.area !== undefined
  );
  const chunks = [];
...
export default function calculateDitchVolumes(params: Params) {
  ...
  const data_pts = surveyPoints
    .map((pt) => processPoint(params, pt))
    .sort((a, b) => (a.chainage || 0) - (b.chainage || 0));
  const data_rng = calcVolumeChunks(data_pts);

  const chainages = data_pts
    .filter((i) => i.chainage !== undefined)
    .map((i) => i.chainage!);
